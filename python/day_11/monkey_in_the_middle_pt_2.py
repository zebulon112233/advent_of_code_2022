...
def readMonkeyData(firstLine):
    ...
    ID = readID(firstLine)
    items = readItems()
    operation, operator = readOperation()
    test = readTest()

    readNextLine() # read empty line
    
    return ID, items, operation, operator, test
...
class Monkey():
    ...
    def __init__(self,data):
        (ID, items, operation, operator, test) = data
        self.ID = ID
        self.items = items
    ...
    def reduceWorryLevel(self, worryLevel):
        ...
        for monkey in Monkey.collection:
            gcdOfMonkeyTest *= monkey.test["modulo"]
            
        return worryLevel % gcdOfMonkeyTest
